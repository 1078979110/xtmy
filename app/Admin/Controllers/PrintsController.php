...
class PrintsController extends AdminController{
    public function jxsPrint(Content $content){
        ...
        foreach ($fenPi as $key=>$value){
            ...
            $data[$key] = [
                ...
                'registivalidate' => ($medicinalInfo->registivalidate != '长期有效')?date('Y/m/d', strtotime($medicinalInfo->registivalidate)):'长期有效',
    ...
    public function hospitalPrint(Content $content){
        ...
        $headername['xzb'] =['采购订单号：','订单日期：','供应商编码及名称：V30022 武汉协同贸易有限公司','收货单位：','送货人：','送货日期：','收货人：','收货日期：'];
